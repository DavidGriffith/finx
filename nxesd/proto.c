...
/*                                                                        */
/* NXESD, NX protocol compression and NX extensions to this software      */
...
{
  int ok;
  ESDBG_TRACE( printf( "auth debug: owner_only: %i esd_is_owned: %i esd_is_locked: %i submitted_key: %s\n", 
		     owner_only, esd_is_owned, esd_is_locked, submitted_key ); );

  /*
   * We are binding only 127.0.0.1 so we don't need authorization
   */
  ok = 1;
