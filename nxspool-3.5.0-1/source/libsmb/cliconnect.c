...
BOOL cli_session_request(struct cli_state *cli,
			 struct nmb_name *calling, struct nmb_name *called, BOOL *sessionReq)
{
	char *p;
	int len = 4;
	...
	if (!sessionReq || (sessionReq!=NULL && (!(*sessionReq))) || cli->port == 445)
		return True;
	...
	if (CVAL(cli->inbuf,0) == 0x84) {
		...
		*/
		...
		/* Try again */
		{
			...
			depth++;
			ret = cli_session_request(cli, calling, called, sessionReq);
			depth--;
			return ret;
		}
	} /* C. Hoch 9/14/95 End */
...
again:
	...
	if (!cli_session_request(cli, &calling, &called, sessionReq)) {
		char *p;
                 ...
                 BOOL *sessionReq) 
{
	...
	*output_cli = NULL;
	...
	nt_status = cli_start_connection(&cli, my_name, dest_host, 
					 dest_ip, port, signing_state, flags, retry, sessionReq);
                                     ...
                                     struct in_addr *pdest_ip)
{
    ...
    if (!cli_session_request(*ppcli, &calling, &called, NULL)) {
		NTSTATUS status;
...
with error %s.\n", desthost, cli_errstr(*ppcli) ));
		...
		*ppcli = cli_initialise();
		...
		if (!NT_STATUS_IS_OK(status) ||
				!cli_session_request(*ppcli, &calling, &smbservername, NULL)) {
                                         ...
                                         struct user_auth_info *user_info)
{
	...
	nt_status = cli_full_connection(&cli, myname, server, server_ip, 0, "IPC$", "IPC", 
					...
					CLI_FULL_CONNECTION_ANONYMOUS_FALLBACK, Undefined, NULL, NULL);
