...
gkrellmd_load_monitors(void)
	...
	if (_GK.list_plugins)
		exit(0);
	if (_GK.log_plugins && plugin_install_log)
...
gkrellmd_update_monitors(void)
	...
	if (GK.second_tick)
		...
		GK.two_second_tick  = 1;//((pCur->tm_sec % 2) == 0) ? TRUE : FALSE;
		GK.five_second_tick = 1;//((pCur->tm_sec % 5) == 0) ? TRUE : FALSE;
		GK.ten_second_tick  = 1;//((pCur->tm_sec % 10) == 0) ? TRUE : FALSE;
...
gint
gkrellmd_update_monitors_delay(void)
	{
	GList			*list, *c_list;
	GkrellmdMonitor			*mon;
	GkrellmdMonitorPrivate	*mp;
	...
	if (GK.second_tick)
	{
                GK.second_tick = TRUE;
		pCur = localtime(&_GK.time_now);
                GK.two_second_tick =  TRUE;
                GK.five_second_tick = TRUE;
                GK.ten_second_tick =  TRUE;
                GK.minute_tick     = (pCur->tm_min  != gkrellmd_current_tm.tm_min);
		gkrellmd_current_tm = *pCur;
