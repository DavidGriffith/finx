  ...
  switch (_kernelStep)
  {
    ...
    case 1:
    {
      ...
      if (setsockopt(fd, SOL_IP, IP_TOS, &flag, sizeof(flag)) < 0)
      {
        ...
        {
          #ifdef PANIC
          *logofs << "Socket: PANIC! Failed to set IPTOS_LOWDELAY flag on "
                  << "FD#" << fd << ". Error is " << EGET() << " '" << ESTR()
                  << "'.\n" << logofs_flush;
          #endif

          cerr << "Error" << ": Failed to set IPTOS_LOWDELAY flag on "
               << "FD#" << fd << ". Error is " << EGET() << " '" << ESTR()
               << "'.\n";
        }
...
int GetBytesReadable(int fd)
{
  ...
  //
  // A failure is sometimes what we want,
  // for example at session shutdown.
  //

  if (ioctl(fd, FIONREAD, (void *) &readable) < 0)
  {
    #ifdef TEST
  ...
  #ifdef TEST
  *logofs << "Socket: Returning " << readable
...
int GetBytesQueued(int fd)
{
  ...
  // 2.0 and 2.2 (like current MIPS for PS/2). This
  // function is used to detect proxy congestion, so
  // it is only critical at X client side.
  //
